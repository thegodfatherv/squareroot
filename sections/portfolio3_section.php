...
while ( $gallery->have_posts() ): $gallery->the_post();
	...
	$list .= '<figure class="effect-layla">';

	$thumb_id  = get_post_thumbnail_id();
	$thumb_url = wp_get_attachment_image_src( $thumb_id, 'thumbnail-size', true );
	$list .= '<img src="' . $thumb_url[0] . '" alt="placeholder-img">';

	$title     = get_post_meta( get_the_ID(), 'project_client_name', true );//get_the_title();
	$sub       = get_post_meta( get_the_ID(), 'project_client_sub_name', true ); //get_the_content();
	$link      = $thumb_url[0];
	$post_link = get_permalink();
	$list .= '<figcaption><h2>' . $title . '</h2><p>' . $sub . '</p><a href="javascript:void(0)" rel="external" class="portfolio-image ' . $post->ID . '">' . $title . '<span><em>' . $sub . '</em></span></a></figcaption>';
	$list .= '</figure>';
	$list .= '</li>';
endwhile;
wp_reset_postdata();
